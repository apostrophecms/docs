...
- **Templates, in [JSX](/guide/jsx-templates.md) or [Nunjucks](/guide/nunjucks-templates.md).** Page, widget, and component templates can be written as JSX — real JavaScript, with modern editor support and no separate front-end project required — or in Nunjucks, a richly featured template language with a syntax nearly identical to Twig, Jinja, and other Django-inspired templating languages. The two interoperate, so a project can mix them or migrate from one to the other incrementally.
...
| `/views` | Template files that do not belong to any one module. Apostrophe looks for site wrapper templates here, including `layout.html` or `layout.jsx`. |
