...
| `prefix` | If explicitly set to `false`, do not automatically prefix the url. It can become handy when the given url it already prefixed,  which is the case when using the documents computed `_url` field for instance |
