...
::: tip Before you write a custom editor
If what you actually want is for the editor to type a heading or a paragraph of the widget on the page, you do not need any of this. A widget's `string` and `richText` fields can be rendered in place by the template and edited there, with no `contextual: true` and no Vue component — one line of template code per field. See [inline editing](/guide/inline-editing.md). Reach for the pattern below when the *whole* widget needs an editing interface of its own, as the image widget does.
:::
...
The Vue component name configured via the `widgetEditor` subproperty of the `components` option (the "widget editor") must display the content of the widget in an editable form. To provide a contextual editing experience, that interface should be similar to the ordinary read-only view generated by the widget's own template.
...
This is a straightforward widget template, as might be found in any custom widget. The presentation is intentionally similar to that used in the widget editor component, below. The equivalent in Nunjucks would be a `widget.html` with the same markup:
