...
The `@apostrophecms/search` module template can be customized through adding files to the project-level `modules/@apostrophecms/search/views` folder. The default template, `index.html`, is written in Nunjucks. For a JSX project, create an `index.jsx` in that folder and use the core template as a reference for the data it receives. A project-level template takes precedence over the core one regardless of extension. To stay in Nunjucks instead, copy `index.html` to your project-level folder and modify it. The main block of this template contains a form for adding any filters to the search if the `filters` option has been populated, as well as the query terms. Below the form is a section to output the search results returned to the page in the `data.doc` object. Finally, the page includes the [standard pagination section](/guide/piece-pages.md#pagination). In the core template, it is added using `{% include "pager.html" %}` to load it in from another file in the `views` folder so that it can be easily reused. In JSX, the equivalent is to import a pager component, as shown in the [piece pages guide](/guide/piece-pages.md#pagination). You can elect to add the pager directly to your project template or keep it as a separate file.
