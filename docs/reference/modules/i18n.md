...
The module makes an instance of the [i18next](https://npmjs.org/package/i18next) npm module available in both JSX and Nunjucks templates via the `__t()` helper function. That function is also available on `req` objects as `req.t()` and in user interface Vue.js components as `this.$t()`.
