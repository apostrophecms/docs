...
- If *any* locale has a `hostname` setting *one* of these must be true:
  1. **The Apostrophe app must have a `baseUrl` set in the `app.js` file** (a best practice in most cases anyway) or set through the `APOS_BASE_URL` environment variable, OR
