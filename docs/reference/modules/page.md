...
If a requested URL is not found and the requested URL contains uppercase letters, by default the `@apostrophecms/page` module will convert the *entire*  requested URL to lowercase and attempt to redirect to this altered URL. Setting the `redirectFailedUppercaseUrls` to false will override this behavior.
