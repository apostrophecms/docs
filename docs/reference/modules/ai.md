...
| *adapter settings* | Any extra key the entry's adapter declares — a service fact only that dialect knows. The `anthropic` adapter declares [`workspaceId`](#adapter-settings). Anything else fails the boot. |

#### Adapter settings

Some services need one more fact about the credential than a key. Anthropic is the shipped example: an identity-linked API key that is **not** scoped to a single workspace must send the workspace it acts in on every request (the `anthropic-workspace-id` header), or the API answers `400 … anthropic-workspace-id is required`. A workspace-scoped key never needs it.

So the `anthropic` adapter declares a `workspaceId` setting, and any entry it speaks for may carry it:
...
Settings follow the same rules as the key: the environment variable the adapter names (`APOS_ANTHROPIC_WORKSPACE_ID`) overrides the configured value, and the value must be a string.

Because each adapter declares exactly the settings it reads, an entry key that is neither standard nor declared is a typo — the boot fails and names it, instead of the request failing later for a reason you would have to debug at the provider.

::: warning
Unlike the key, a setting's variable name **cannot** be renamed with `envKey`. See [A setting with your own environment variable](#a-setting-with-your-own-environment-variable).
:::

Adapters and the settings they declare are listed in each adapter's reference page; today only [`anthropic`](/reference/modules/ai-adapter-anthropic.md#adapter-settings) declares one.
...
#### A setting with your own environment variable

`envKey` in a provider entry renames the variable for the **api key only**. A setting's variable name is fixed by the adapter's declaration — there is no per-entry rename. When your deployment already holds the value in a variable of its own, you have two routes.

**Read your variable in the configuration.** Configuration is assembled at runtime, so the entry can read any variable directly:
...
Be sure the adapter's own variable is not set. The environment always wins: if `APOS_ANTHROPIC_WORKSPACE_ID` is present and non-empty, it overrides the configured value — including one you just read from `MY_CLAUDE_WORKSPACE`. This route only works when the declared variable is absent from the deployment environment.

**Or rename the declared variable.** The adapter definition is built by the adapter module's `adapter()` method, an ordinary override seam. Extending it renames the variable for real — the adapter then reads yours and never looks at its default:
...
Prefer the first route for a one-off; prefer the rename when the declared variable might legitimately exist in your environment for something else, or when you cannot guarantee its absence.
...
The named provider must be configured and declare the `image` capability, or the boot fails. `quality` accepts `low`, `medium` or `high`.

`aspect` accepts a `W:H` ratio, or one of three named tokens that ground to conventional photo ratios:
...
::: warning
`landscape` is **4:3**, not 16:9. For a widescreen image, pass the ratio itself — `aspect: '16:9'`.
:::

Whichever form is used, the value resolves to the nearest ratio the routed model declares, and the resolved ratio comes back on the result.
...
| `APOS_ANTHROPIC_WORKSPACE_ID` | The `anthropic` adapter's [`workspaceId` setting](#adapter-settings): the workspace requests act in, required by Anthropic with an identity-linked multi-workspace key. Overrides a configured `workspaceId`. |
...
- an entry key its adapter does not declare — `"providers.anthropic.workspaceID" is neither a standard entry key nor a setting of adapter "anthropic"`. Note the typo it just caught
...
| `onMessage` | function | Called and awaited with each intermediate assistant turn, before its tools run. See [Hooks](#hooks). |
| `onToolCall` | function | Called and awaited twice around every tool handler — `phase: 'start'` and `'end'`. See [Hooks](#hooks). |

An unknown option name is an `invalid` error. Bad input is rejected before any provider is touched.

#### Hooks

Both hooks are awaited, so the round reports itself while it happens rather than when it is over. A throw from either stops the call — with one exception, noted below.

**`onMessage(message, meta)`**

Called with each *intermediate* assistant turn — one whose tool requests the loop is about to execute — and awaited before those tools run. The final answer is not reported here; it is the return value.

```javascript
async onMessage(message, meta) {
  // message: the assistant turn, { role, content }
  // meta.step: the model turn it came from, counting from 1
}
```

**`onToolCall(event)`**

Called twice around every tool handler the loop runs.

| `event` property | Meaning |
|---|---|
| `phase` | `'start'` before the handler, `'end'` after it. |
| `call` | The request, exactly as the model made it: `{ id, name, input }`. |
| `step` | The model turn that asked for it, counting from 1. Continued runs report their trailing calls at step `0`. |
| `result` | On `'end'` only, when the handler returned. |
| `error` | On `'end'` only. A recoverable failure is the message the model was told; a hard-stopping one is the message about to be thrown. |
| `suspended` | On `'end'` only: the ask of a handler that paused the run. |

A call naming no registered tool never starts and is not reported. A throw from `onToolCall` stops the call, **except** from the end report of a handler whose own failure is already stopping it — that throw is logged as a [`hook` record](#the-event-types) instead of replacing the failure in flight.

::: info
Under [`generateJob`](#async-generatejob-req-prompt-options-options) both hooks receive `{ jobId }` merged into their meta — so `onToolCall` takes a second argument there that it does not take in a plain `generate`.
:::
...
| `aspect` | A `W:H` ratio, or `square` (`1:1`), `portrait` (`3:4`) or `landscape` (`4:3`). Resolved to the nearest ratio the routed model declares. See [the image route](#image) — `landscape` is 4:3, not 16:9. |
...
The [hooks](#hooks) are passed through untouched except that `{ jobId }` is merged into their meta: `onMessage(message, { step, jobId })`, and `onToolCall(event, { jobId })` — a second argument the hook does not receive in a plain `generate`.
...
apos.bus.$on('ai-generate-job', (data) => {
  ...
  switch (data.stage) {
    ...
    case 'ended': /* data.status: 'completed' | 'canceled' | 'failed', plus finishReason or error */ break;
  }
});
```
...
::: warning
The terminal status is spelled `canceled`, with one `l`, while `finishReason` on the result is `cancel`. Compare against the exact strings.
:::
...
Either way the flag travels through the job record, the abort signal reaches the in-flight provider call and every handler, the run winds down per `generate`'s cancellation semantics with the partial result stored, and the job ends `canceled`.
...
Some doc types are refused to the AI outright, **whatever the user's own permissions**, for every action — `view` included:

| Type | |
|---|---|
| `@apostrophecms/user` | Accounts |
| `@apostrophecms-pro/advanced-permission-group` | Permission grants |

These are where a model's mistake is a privilege escalation, so the refusal is unconditional and not configurable. It applies whether the type is named as a string or passed as a document.

The practical consequence for tool authors: a tool cannot read user documents to answer a question about people. A feature that legitimately needs user data ships a narrow tool that returns exactly the fields it needs, rather than handing the model the document.

```javascript
self.apos.ai.can(req, 'view', '@apostrophecms/user'); // false, always
```
...
| Method | Meaning |
|---|---|
| `addAdapter(adapter)` | Register an adapter. Called from the adapter module's `init`. Re-registering a name overrides it, so a custom adapter can replace a standard one. |
| `getAdapter(name)` | The adapter registered under `name`, or `undefined`. |
...
| `pause(ms)` | Await a delay. Useful for spacing requests an adapter fans out — the `google` adapter uses it to stagger the concurrent calls a multi-image request becomes. |
...
**The envelope.** Records carry the standard Apostrophe log fields — `module`, `type`, `severity` and the message exactly as written; the module name and event type are **fields, not a message prefix** — plus the request fields, because `req` is always passed: `url`, `path`, `method`, `ip`, `query` and `requestId`. That last one is Apostrophe's per-request id, so every record of one editor action correlates.
...
| `image-partial` | `error` | An image batch delivered fewer images than requested: some of the parallel provider requests failed, but not all, so the call still succeeds with what arrived. |
...
| `watcher` | `error` | A background job's cancellation watcher died. The run continues, but a cancel request can no longer stop it before it finishes. |
...
`image-partial` carries `provider`, `model`, `code`, `status`, `kind`, plus `requested` and `delivered`. `notify` carries `jobId`, `stage` and `stack`. `hook` carries `stack` plus `jobId` and `hook: 'onEnd'`, or the tool name. `watcher` carries `jobId` and `stack`.
...
**The swallowed failures matter most.** `notify`, `hook`, `watcher` and `image-partial` all record something that did *not* fail the run: a broken progress transport, a throwing hook, a dead cancellation watcher and a partial image batch are all survivable by design. That is exactly why they are logged — without the record they would be invisible. If a user reports "the progress bar froze but the job finished", `notify` is the first thing to look at.
...
'@apostrophecms/log': {
  options: {
    filter: {
      '*': { severity: [ 'error' ] },
      '@apostrophecms/ai': { events: [ 'retry', 'failure', 'image-partial' ] }
    }
  }
}
```
